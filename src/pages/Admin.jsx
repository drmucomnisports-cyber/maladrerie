    ...
    <div className="min-h-screen bg-[#F8F8F8] font-sans p-8">
      <div className="max-w-7xl mx-auto relative">
        <div className="bg-[#F8F8F8] pb-8 border-b border-slate-200 shadow-sm mb-8">
          ...
          {adminFeedback && (
            <div className={`mb-4 px-4 py-3 rounded-lg border shadow-sm flex items-center gap-3 ${
              adminFeedback.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-green-50 border-green-200 text-green-800'
            }`}>
                    ...
                    <td className="p-4">
                      <div className="space-y-2">
                        <select 
                          ...
                          className={`w-full text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg outline-none border-2 cursor-pointer ${
                            res.statut === 'EN_ATTENTE' ? 'border-amber-200 bg-amber-50 text-amber-700' :
                  ...
                  <div className="space-y-3">
                    {Object.entries(missionChecks).map(([type, val]) => (
                      <label key={type} className={`flex items-center justify-between p-3 rounded-lg border-2 cursor-pointer transition-all ${
                        val.checked ? 'border-muc-blue bg-muc-blue/5' : 'border-slate-200 bg-white hover:border-slate-300'
                      }`}>
