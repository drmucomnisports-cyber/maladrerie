  ...
  try {
    ...
    try {
      ...
      if (devisFinal.repas) {
        ...
        Object.entries(devisFinal.repas).forEach(([key, val]) => {
      ...
      const optionsLignes = devisFinal.options ? Object.entries(devisFinal.options).filter(([k, v]) => v).map(([k, v]) => {
        let optNom = k;
        let optPrix = 0;
        let qte = 1;
        if (k === 'menage') {
          ...
          optPrix = 5;
          qte = (devisFinal.occupants && devisFinal.occupants.length) || totalPersonnes || 1;
      ...
      let promoMontant = 0;
      if (devisFinal.codePromo) {
        const promo = await prisma.promoCode.findUnique({ where: { code: devisFinal.codePromo.toUpperCase() } });
        if (promo && promo.actif) {
          if (promo.type === 'pourcentage') {
            const ratio = 1 - (promo.valeur / 100);
            if (ratio > 0) {
              const prixSansPromo = devisFinal.prixTotal / ratio;
              promoMontant = prixSansPromo - devisFinal.prixTotal;
            }
          } else {
      ...
      const pdfData = {
        ...
        adminEmail: resolvedAdminEmail,
        adminTel: resolvedAdminTel,
        chambres: devisFinal.chambres.map(id => CHAMBRES_NAMES[id] || `Chambre ${id}`),
        nuits,
        detailsLignes,
        ...
        prixTotal: devisFinal.prixTotal,
        montantAcompte: devisFinal.montantAcompte || (Math.round((Math.max(0, devisFinal.prixTotal - calculerTotalRepasServeur(devisFinal.repas)) * 0.3 + calculerTotalRepasServeur(devisFinal.repas)) * 100) / 100),
        promoCode: devisFinal.codePromo || null,
        promoMontant
      };
      ...
      await sendMail({
        to: devisFinal.client.email,
        subject: `Prolongation de votre devis - Séjour Gîte de La Maladrerie`,
        attachments: [
          {
            content: pdfBuffer.toString('base64'),
            name: `Devis_${devisFinal.numeroDevis}.pdf`
        ...
        html: `
          <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: #f4f4f4; padding: 20px;">
            <tr>
              <td align="center">
                <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #dddddd; font-family: sans-serif;">
                  <tr>
                    <td style="background-color: #004B93; padding: 30px; text-align: center;">
                      <h1 style="color: #ffffff; margin: 0;">Gîte de La Maladrerie</h1>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding: 40px; color: #333333; line-height: 1.6;">
                      <h2 style="color: #004B93; margin-top: 0;">Bonjour ${devisFinal.client.nom},</h2>
                      <p>Nous avons le plaisir de vous informer que la validité de votre devis pour votre séjour a été prolongée.</p>
                      <p>Les chambres restent bloquées en votre faveur pour le moment.</p>
                      
                      <table width="100%" style="font-size: 14px; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin: 20px 0;">
                        <tr>
                          <td style="font-weight: bold; width: 120px; border-bottom: 1px solid #eeeeee; padding-bottom: 8px;">N° Devis</td>
                          <td style="border-bottom: 1px solid #eeeeee; padding-bottom: 8px;">${devisFinal.numeroDevis}</td>
                        </tr>
                        <tr>
                          <td style="font-weight: bold; border-bottom: 1px solid #eeeeee; padding: 8px 0;">Période</td>
                          <td style="border-bottom: 1px solid #eeeeee; padding: 8px 0;">Du ${new Date(devisFinal.dateDebut).toLocaleDateString('fr-FR')} au ${new Date(devisFinal.dateFin).toLocaleDateString('fr-FR')}</td>
                        </tr>
                        <tr>
                          <td style="font-weight: bold; border-bottom: 1px solid #eeeeee; padding: 8px 0;">Montant</td>
                          <td style="border-bottom: 1px solid #eeeeee; padding: 8px 0; font-weight: bold; color: #004B93;">${devisFinal.prixTotal.toFixed(2)} €</td>
                        </tr>
                        <tr>
                          <td style="font-weight: bold; padding-top: 8px;">Nouvelle Expiration</td>
                          <td style="padding-top: 8px; font-weight: bold; color: #dc2626;">${new Date(devisFinal.expireLe).toLocaleDateString('fr-FR', {hour: '2-digit', minute:'2-digit'})}</td>
                        </tr>
                      </table>

                      <div style="background-color: #fff3cd; border: 1px solid #ffeeba; padding: 15px; border-radius: 8px; font-size: 14px; color: #856404; margin-bottom: 25px;">
                        ⚠️ <strong>Important :</strong> La disponibilité des chambres n'est garantie que jusqu'au <strong>${new Date(devisFinal.expireLe).toLocaleDateString('fr-FR', {hour: '2-digit', minute:'2-digit'})}</strong>.
                      </div>
      ...
      console.error("Erreur envoi email devis prolong:", err);
    }
