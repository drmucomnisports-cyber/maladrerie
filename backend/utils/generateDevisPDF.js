...
/**
 * Génère un buffer PDF pour un devis
 * @param {Object} data 
...
async function generateDevisPDF(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // --- HEADER ---
            // On tente d'afficher un en-tête stylisé
            ...
            doc.text('DESIGNATION', leftCol + 10, tableTop + 6);
            doc.text('DÉTAILS', leftCol + 150, tableTop + 6);
            ...
            doc.text(`${data.chambres.join(', ')}`, leftCol + 150, y, { width: 280 });
            doc.text(`${data.prixSejour.toFixed(2)}`, leftCol + 440, y, { align: 'right', width: 60 });
            y += 25;

            // Ligne Taxe
            doc.text('Taxe de séjour', leftCol + 10, y);
            doc.text(`4% par nuitée par adulte`, leftCol + 150, y);
            doc.text(`${data.taxeSejour.toFixed(2)}`, leftCol + 440, y, { align: 'right', width: 60 });
            y += 25;

            // Options
            if (data.options && data.options.length > 0) {
                data.options.forEach(opt => {
                    doc.text(`Option : ${opt.nom || 'Service'}`, leftCol + 10, y);
                    doc.text(`${opt.prix.toFixed(2)}`, leftCol + 440, y, { align: 'right', width: 60 });
                    y += 20;
                });
            }
            ...
            // --- TOTAUX ---
            doc.moveTo(leftCol + 350, y).lineTo(leftCol + 512, y).stroke('#EEEEEE');
            y += 10;
            doc.fontSize(12).font('Helvetica-Bold').text('TOTAL TTC', leftCol + 350, y);
            doc.text(`${data.prixTotal.toFixed(2)} €`, leftCol + 440, y, { align: 'right', width: 60 });
            
            y += 25;
            doc.fontSize(10).font('Helvetica').text('Acompte (30%) à régler :', leftCol + 330, y);
            ...
            // --- PIED DE PAGE ---
            doc.fontSize(8).fillColor('#999999').text('Gîte de la Maladrerie - MUC OMNISPORTS | SIRET: 77555234100015', 0, 750, { align: 'center', width: 612 });
